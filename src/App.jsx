...
const steps = [
  { id: 'signal', number: 1, title: 'Signal', prompt: 'What is arising?', detail: 'Name the event, tension, request, opportunity, or drift before deciding what it means.', field: 'signal' },
  { id: 'invitation', number: 2, title: 'Invitation', prompt: 'What is being offered?', detail: 'Convert command energy into a clear offer with a refusal path.', field: 'invitation' },
  { id: 'consent', number: 3, title: 'Consent', prompt: 'Who or what is affected?', detail: 'Identify affected parties, permission, refusal paths, and valid authority.', field: 'consent' },
  { id: 'alignment', number: 4, title: 'Alignment', prompt: 'Does this preserve coherence?', detail: 'Check values, harms, claims, evidence, reversibility, and repair.', field: 'alignment' },
  { id: 'flow', number: 5, title: 'Flow', prompt: 'What action moves?', detail: 'Take the smallest accountable action that keeps correction possible.', field: 'action' },
  { id: 'receipt', number: 6, title: 'Receipt', prompt: 'What changed and why?', detail: 'Record what was decided, what evidence supported it, and what remains unknown.', field: 'outcome' },
  { id: 'renewal', number: 7, title: 'Renewal', prompt: 'What repairs or regenerates?', detail: 'Close the loop through repair, learning, rollback, apology, or adjustment.', field: 'renewal' },
];
...
const samples = [
  ...
  { title: 'Product team considers a surprise pricing change', domain: 'Product decision', text: 'A product team wants to change pricing quickly, but the change could surprise existing users, increase support load, and weaken trust if no opt-out or explanation exists.' },
  { title: 'Meeting has too much heat to decide cleanly', domain: 'Other', text: 'A meeting has become emotionally intense. People are talking over each other, the facts are mixed with assumptions, and the group needs a way to pause without abandoning accountability.' },
];
...
const guidedSteps = [
  { id: 'context', title: 'Choose context', stepId: 'signal', kind: 'context', helper: 'Start by naming the kind of decision, repair, reflection, or meeting you are working with.' },
  { id: 'signal', title: 'Name the signal', stepId: 'signal', kind: 'receipt', field: 'signal', question: 'What is arising before interpretation, blame, urgency, or command energy gets added?' },
  { id: 'invitation', title: 'Shape the invitation', stepId: 'invitation', kind: 'receipt', field: 'invitation', question: 'What is being offered, and what refusal path keeps participation clean?' },
  { id: 'affected', title: 'Map affected parties', stepId: 'consent', kind: 'receipt', field: 'affectedParties', question: 'Who is directly affected, downstream affected, or responsible for records and repair?' },
  { id: 'consent', title: 'Clarify consent', stepId: 'consent', kind: 'receipt', field: 'consent', question: 'What consent, authority, opt-out, pause, or refusal path exists?' },
  { id: 'gates', title: 'Run the gates', stepId: 'alignment', kind: 'gates', helper: 'Mark each gate as pass, watch, or block. Watch and block items should get repair or reversibility paths.' },
  { id: 'alignment', title: 'Record alignment', stepId: 'alignment', kind: 'receipt', field: 'alignment', question: 'Does the move preserve consent, evidence, repair, coherence, and future play?' },
  { id: 'repair', title: 'Build repair path', stepId: 'renewal', kind: 'repair', helper: 'Name drift, stabilization, affected repair, reversible action, repair action, follow-up, and renewal receipt.' },
  { id: 'flow', title: 'Choose smallest move', stepId: 'flow', kind: 'receipt', field: 'action', question: 'What is the smallest accountable action that can move while keeping correction possible?' },
  { id: 'receipt', title: 'Settle the receipt', stepId: 'receipt', kind: 'receipt', field: 'outcome', question: 'What changed, why, and what remains unknown after the action?' },
  { id: 'renewal', title: 'Close with renewal', stepId: 'renewal', kind: 'receipt', field: 'renewal', question: 'What follow-up, repair, rollback, learning, or check-in keeps the field alive?' },
  { id: 'evidence', title: 'Evidence boundary', stepId: 'receipt', kind: 'evidence', helper: 'Separate what is known, assumed, and still unknown before saving or sharing.' },
  { id: 'finish', title: 'Finish and save', stepId: 'receipt', kind: 'finish', helper: 'Copy, download, or save the receipt. Nothing leaves this browser unless you export it.' },
];
...
function fieldCompletion(fields, repairPlan, gateStates) {
  const receiptKeys = ['signal', 'invitation', 'affectedParties', 'consent', 'alignment', 'action', 'outcome', 'renewal', 'evidence', 'assumptions', 'unknowns'];
  const repairKeys = Object.keys(repairPlan);
  const filled = receiptKeys.filter((key) => String(fields[key] || '').trim().length > 0).length + repairKeys.filter((key) => String(repairPlan[key] || '').trim().length > 0).length + gateStates.filter((gate) => gate.status !== 'watch' || gate.note).length;
  const total = receiptKeys.length + repairKeys.length + gateStates.length;
...
function makeReceipt(scenario, scenarioType, fields, repairPlan, reversibilityAssessment, gateStates, shareUrl, completion) {
...
export default function App() {
  ...
  const currentGuide = guidedSteps[Math.min(guidedIndex, guidedSteps.length - 1)];
  ...
  const completion = useMemo(() => fieldCompletion(receiptFields, repairPlan, gateStates), [receiptFields, repairPlan, gateStates]);
  ...
  const receipt = useMemo(() => makeReceipt(scenario, scenarioType, receiptFields, repairPlan, reversibilityAssessment, gateStates, shareUrl, completion), [scenario, scenarioType, receiptFields, repairPlan, reversibilityAssessment, gateStates, shareUrl, completion]);
  ...
  const goGuide = (next) => { const bounded = Math.max(0, Math.min(guidedSteps.length - 1, next)); setGuidedIndex(bounded); setActive(guidedSteps[bounded].stepId); };
  const applySample = (sample) => { setScenario(sample.text); setScenarioType(sample.domain); setActive('signal'); setGuidedIndex(1); setRepairPlan(repairTemplates[sample.domain] || starterRepairPlan); };
  ...
  const scenarioSnapshot = () => ({ id: `scenario-${hash(JSON.stringify({ scenario, scenarioType, receiptFields, repairPlan, gateStates }))}`, title: scenario.slice(0, 82) || 'Untitled scenario', scenario, scenarioType, active, guidedIndex, receiptFields, repairPlan, gateStates, updatedAt: new Date().toISOString() });
  ...
  const loadScenario = (record) => { setScenario(record.scenario || ''); setScenarioType(record.scenarioType || 'Other'); setActive(record.active || 'signal'); setGuidedIndex(record.guidedIndex || 0); setReceiptFields(record.receiptFields || starterReceipt); setRepairPlan(record.repairPlan || starterRepairPlan); setGateStates(record.gateStates || gates.map(([id]) => ({ gateId: id, status: 'watch', note: '' }))); flash('Scenario loaded'); };
  ...
  const renderGuidedBody = () => {
    if (currentGuide.kind === 'context') return <div className='guidedEditor'><label>Scenario type<select value={scenarioType} onChange={(event) => { setScenarioType(event.target.value); setRepairPlan(repairTemplates[event.target.value] || starterRepairPlan); }}>{scenarioTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select></label><label>Scenario<textarea value={scenario} onChange={(event) => setScenario(event.target.value)} /></label></div>;
    if (currentGuide.kind === 'receipt') return <label className='guidedQuestion'>{currentGuide.question}<textarea value={receiptFields[currentGuide.field] || ''} onChange={(event) => setField(currentGuide.field, event.target.value)} /></label>;
    if (currentGuide.kind === 'gates') return <div className='gates guidedGates'>{gates.map(([id, title, prompt]) => { const gate = gateStates.find((item) => item.gateId === id) || { status: 'watch', note: '' }; return <article key={id} className={gate.status}><div><h3>{title}</h3><p>{prompt}</p></div><label>Status<select value={gate.status} onChange={(event) => setGate(id, { status: event.target.value })}><option value='pass'>Pass</option><option value='watch'>Watch</option><option value='block'>Block</option></select></label><label>Note<input value={gate.note} onChange={(event) => setGate(id, { note: event.target.value })} /></label></article>; })}</div>;
    if (currentGuide.kind === 'repair') return <div><div className='repairToolbar'><button onClick={applyRepairTemplate}>Apply {scenarioType} template</button><button className='secondary' onClick={draftRepairFromScenario}>Draft from scenario</button></div><div className='fields repairFields'>{Object.entries(repairPlan).map(([key, value]) => <label key={key}>{key}<textarea value={value} onChange={(event) => setRepairField(key, event.target.value)} /></label>)}</div></div>;
    if (currentGuide.kind === 'evidence') return <div className='guidedEditor three'><label>Evidence<textarea value={receiptFields.evidence} onChange={(event) => setField('evidence', event.target.value)} /></label><label>Assumptions<textarea value={receiptFields.assumptions} onChange={(event) => setField('assumptions', event.target.value)} /></label><label>Unknowns<textarea value={receiptFields.unknowns} onChange={(event) => setField('unknowns', event.target.value)} /></label></div>;
    return <div className='guidedFinish'><p>This guided pass is ready to save, copy, or export. The receipt keeps the claim boundary, gate summary, repair plan, and reversibility guidance together.</p><div className='practiceActions'><button onClick={() => copyText(markdownReceipt).then(() => flash('Guided receipt copied'))}>Copy receipt</button><button className='secondary' onClick={() => downloadText(`${receipt.id}.md`, markdownReceipt, 'text/markdown')}>Download MD</button><button className='secondary' onClick={() => downloadText(`${receipt.id}.json`, JSON.stringify(receipt, null, 2), 'application/json')}>Download JSON</button><button className='secondary' onClick={saveReceipt}>Save locally</button></div></div>;
  };
  ...
  return (
    <main>
      ...
      <section className='hero'>
        <div><p className='eyebrow'>Flow Covenant Runtime v{appVersion}</p><h1>A Playfield Governance Model for Coherent Systems</h1><p className='lead'>Move from force-first law thinking into participation-first flow thinking.</p><div className='formula'>Force creates compliance. Flow creates participation.</div><nav className='actions' aria-label='Page sections'><a href='#about'>About</a><a href='#guide'>Guide</a><a href='#loop'>Explore loop</a><a href='#simulator'>Simulator</a><a href='#gates'>Gates</a><a href='#repair'>Repair</a><a href='#practice'>Practice</a><a href='#receipt'>Receipt</a><a href='#library'>Library</a></nav></div>
      ...
      <section className='panel guidedMode' id='guide'><p className='eyebrow'>Guided Mode</p><h2>Walk the whole loop one clean step at a time</h2><p className='sectionLead'>Guided Mode turns the full Flow Covenant process into a wizard. It is designed for first-time users, meeting facilitators, and anyone who wants less overwhelm and more clarity.</p><div className='guidedProgress'><span><b>{completion.percent}%</b> complete</span><progress value={completion.percent} max='100'>{completion.percent}</progress><span>{completion.filled}/{completion.total} fields touched</span></div><div className='guidedLayout'><aside className='guidedRail'>{guidedSteps.map((item, index) => <button key={item.id} className={index === guidedIndex ? 'active' : ''} onClick={() => goGuide(index)}><span>{index + 1}</span>{item.title}</button>)}</aside><article className='guidedCard'><p className='eyebrow'>Step {guidedIndex + 1} of {guidedSteps.length}</p><h3>{currentGuide.title}</h3><p>{currentGuide.question || currentGuide.helper}</p>{renderGuidedBody()}<div className='guidedMeta'><span>Risk: <b>{evaluation.risk}</b></span><span>Gates: <b>{gateCounts.pass}</b> pass · <b>{gateCounts.watch}</b> watch · <b>{gateCounts.block}</b> block</span><span>Reversibility: <b>{reversibilityAssessment.label}</b></span></div><div className='guidedNav'><button className='secondary' onClick={() => goGuide(guidedIndex - 1)} disabled={guidedIndex === 0}>Back</button><button onClick={() => goGuide(guidedIndex + 1)} disabled={guidedIndex === guidedSteps.length - 1}>Next</button><button className='secondary' onClick={saveScenario}>Save guided session</button><button className='secondary' onClick={() => { setGuidedIndex(0); setActive('signal'); flash('Guided Mode reset to start'); }}>Reset guide</button></div></article></div></section>
